...
//abstract(추상) 메소드 : 실행영역이 없는 메소드, 선언만 되어 있는 메소드
	...
	// 팀장 메소드를 정의해준다. : 회사의 기능구현 메소드를 정의
	  ...
	  //자식클래스에서 재정의 해서 구현부를 만들어 줘야 한다.
	// 예) 메소드의 기본 형식 : void print() {}
	// 추상 메소드 : 실행영역이 없는 메소드 : void print();
...
class B extends A { //콘크리트 클래스 : 추상 클래스에서 선언한 메소드를 모두 구현한 클래스
	...
	@Override
	void cry() {
		System.out.println("으아아아아아아아악!!!!!!!!!!!!");
	}
}

abstract class C extends A { // 부모의 추상 메소드중 일부만 구현 할 경우 : 추상클래스
...
class D extends C { // 콘크리트 클래스 : 부모의 모든 추상메소드를 마지막으로 구현한 클래스
	...
	@Override
	void cry() {
		System.out.println("씨바바바바바ㅏ밥바ㅏㅂ바");

 ...
 * */
		...
		//1. 논리 연산자 AND (&&) : 두 항이 모두 true 일 때
		
		System.out.println(true && true); //true , 쇼트서킷이 발생되지 않는다.
		System.out.println(true && false); //false, 쇼트서킷이 발생 되지 않음.
		System.out.println(false && true); //false, 쇼트서킷이 발생된다.
		System.out.println(true && (5<3)); //false, 쇼트서킷이 발생 되지 않음.
		...
		//숏 서킷을 사용 여부 (논리연산자일때 발생, 비트연산자일때는 발생되지않음)
		int value2 = 3;
		...
		int value5 = 3;
		System.out.println(true | ++value4 > 6); // true
		System.out.println(value4); //4, 쇼트서킷이 발생되지 않는다.

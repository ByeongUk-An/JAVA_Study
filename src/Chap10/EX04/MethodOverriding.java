...
// 메소드 오버라이딩 (MethodOverridng) :
	...
	// 2. 부모 클래스에서 선언된 메소드를 새롭게 정의해서 사용 하는것.
...
class B extends A {
	@Override  // 메소드 오버라이딩시 , 반드시 부모의 메소드를 재정의 하도록 설정.
			   // @Override 어노테이션을 사용하지 않을 경우 : 오타가 나거나 형식이 맞지 않을 경우
			   // 별개의 메소드로 정의됨.
	void print() {
...
public class MethodOverriding {
	...
	public static void main(String[] args) {
		...
		// A 타입 업캐스팅 <==== 중요! : B를 업캐스팅해서 A 타입으로 두면 
		A ab = new B();
		ab.print();			// B 클래스의 print() 출력
			// ab.print()는 A 클래스의 print를 호출하지만 동적바인딩에 의해서 a클래스의 print()가 나오지 않고 B에 있는 print()를 찍어준다.

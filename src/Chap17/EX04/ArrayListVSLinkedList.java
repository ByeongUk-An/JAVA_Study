...
public class ArrayListVSLinkedList {
	public static void main(String[] args) {
		...
		for (int i = 0 ; i < 100000 ; i++) {
			aList.add( 0 ,i );    // 0 번째 방에 계속 해서 10만번째 까지, 계속 뒤로 밀리면서 값이 추가 .  
		}
		...
		System.out.println("LinkedList에서 데이터 추가 시간 " + (endTime - startTime) + "ns");
				///6855500ns    : 250배 정도 빠르다. 
		
		System.out.println("======================================");
		
		//2. 검색시 비교. : Array List가 LinkedList보다 빠르다. 
		//2-1 ArrayList 검색 시간  : 1269700ns
		startTime = System.nanoTime(); 
		for (int i =0 ; i < 100000 ; i++) {
			aList.get(i); 
		}
		...
		//2-2 LinkedList  검색시간  : 4629039200ns
		startTime = System.nanoTime();
		for (int i =0 ; i < 100000 ; i++) {
			linkedlist.get(i); 
		}
		...
		//3-1. ArraryList  : 418425100ns
		startTime = System.nanoTime(); 
		for (int i = 0 ; i < 100000; i++) {
			aList.remove(0);    //index 0 번째자리를 제거 , 값이 당겨온다
		}
		endTime = System.nanoTime(); 
		System.out.println("ArrayList에서 데이터 삭제 시간 " + (endTime - startTime) + "ns");
		
		
		//3-2. LinkedList : 3405500ns
		
		startTime = System.nanoTime(); 
		for (int i = 0 ; i < 100000; i++) {
			linkedlist.remove(0);    //index 0 번째자리를 제거 , 값이 당겨온다
		}
		endTime = System.nanoTime(); 

...
class B {
	int data ;     //두개의 객체가 data 필드의 값이 같을때 , 동일하다라고 식별자를 넣어줘야 한다. 
					// equals(), hashCode() 
	...
	public boolean equals(Object obj) {        //객체의 주소를 비교하는 것이 아니라 객체 내부의 특정 필드의 값을 비교하도록 설정. 
		...
		if ( obj instanceof B) {   				//obj 에 B타입을 내포 하고 있을 때만 다운 캐스팅 , 예외방지 목적(instanceof)
			if (this.data == ((B)obj).data )    //주의 : 잘못 다운캐스팅을 할경우 : 예외가 발생됨, 
			return true ; 
		}
		return false ;
	}
...
////equals()는 재정의  하고 , hashCode() 는  재정의 
class C {
	int data ;     //두개의 객체가 data 필드의 값이 같을때 , 동일하다라고 식별자를 넣어줘야 한다. 
					// equals(), hashCode() 
	...
	public boolean equals(Object obj) {        //객체의 주소를 비교하는 것이 아니라 객체 내부의 특정 필드의 값을 비교하도록 설정. 
		...
		if ( obj instanceof C) {   				//obj 에 B타입을 내포 하고 있을 때만 다운 캐스팅 , 예외방지 목적(instanceof)
			if (this.data == ((C)obj).data )    //주의 : 잘못 다운캐스팅을 할경우 : 예외가 발생됨, 
			return true ; 
		}
		return false ;
	}
...
public class HashSetMachanism {
	public static void main(String[] args) {
		...
		hashSet1.add(a1);    //Set은 중복된 값을 넎지 못한다. 
		hashSet1.add(a2);
		
		System.out.println(hashSet1.size());  //2       
				//< = equals()와 hashCode() 메소드가 재정의 되어있지 많다.  
		
		//2. equals() 재정의 (객체의 data 필드의 값을비교) , hashCode() 메소드가 재정의 되지 않은 상태 
		...
		//3. equals() 재정의 (객체의 data 필드의 값을비교) , hashCode() 메소드가 재정의 
		...
		System.out.println(c1.hashCode() + "," + c2.hashCode());  //data 필드를 기준으로 hashCode 를 생성.
			//두 객체는 동일한 객체이다.
		hashSet3.add(c1);
		hashSet3.add(c2);
		System.out.println(hashSet3.size());   //1   equals(), hashCode() 재정이 되어 있다. data 필드에 대해서. 

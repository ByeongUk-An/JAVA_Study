...
//TreeMap<K,V> : 저장시에 Key를 기준으로 오름차순 정렬되어 저장됨
	//출력시에 오름차순 정렬되어 출력. 
	//Key에 일반 객체를 저장 할때 Comparable<E> compareTo()를 재정의 <== 객체 수정 및 재컴파일, 
...
public class TreeMapMethod_1 {
	public static void main(String[] args) {
		...
		//7. pollFirstEntry() : 첫번째 키와 값 을 끄집어 내어온다. 
		System.out.println(treeMap.pollFirstEntry());
		System.out.println(treeMap);
		//8. poollLastEntry() : 마지막번째 키와 값 을 끄집어 내어온다. 
		...
		NavigableMap<Integer, String> nMap = treeMap.headMap(8, true) ;   //key 8 을 포함하면서. Head 쪽으로 출력
		...
		//12. NavigableMap<K,V> tialMap (K tokey, boolean inclusive)
		NavigableMap<Integer,String> nMap2 = treeMap.tailMap(14, false);    //시작은 포함이 기본, 끝은 미포함이 기본 
		System.out.println(nMap2);
		
		//13. SortedMap<K,V> subMap (K fromKey, K toKey) : 특정 범위 의 값을 저장. 

...
//이너인터페이스 (InnerInterface) : 클래스 내부의 인터페이스가 선언, 외ㅐ부클래스 간단하게 사용할 목적으로
//								클래스 내부에 선언.
//	static 이 자동으로 컴파일러에 의해서 생성. <<== 주의

class A {
	static interface B {	//static 이 컴파일러에 의해서 자동으로 추가됨.
		void bcd();    // public abstract
	}
}

//이너 인터페이스를 구현한 클래스 C  
class C implements A.B {	// A.B는 A클래스 안에 이너인터페이스 B를 뜻한다. / A,B : A인터페이스 와 B인터페이스
	@Override
	public void bcd() {
...
public class InnerInterface_1 {
	...
	public static void main(String[] args) {
		//1. 자식 객체를 직접 생성후 출력 (자식 클래스가 구현 된 경우)

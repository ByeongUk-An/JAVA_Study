...
// 필드의 값 할당. 생성자 호출시 필드에 값을 부여 , p.jangwoo@gmail.com  , 5시 20분 까지
...
public class EX_TryWithResource_3 {
	...
	public static void main(String[] args) {
		...
		try (Abc a1 = new Abc("홍길동", 1111, 80, 90,100);){
			
		}catch (Exception e) {
			
		}  //close()호출 없이도 자동으로 close()
			
		//2. 수동 close() , finnal 블락에서 close() 호출 
		Abc a2 = null;
		try {
			a2 = new Abc("이순신",2222,60,80,100);
		}catch (Exception e) {
			
		}finally {
			if (a2.name != null || a2.studentID != 0 || a2.kor != 0 || a2.eng != 0 || 
					a2.math !=0 || a2.avg != 0.0  ) {

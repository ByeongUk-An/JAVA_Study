...
// 직접 생성한 객체에서 AutoCloseable 을 사용해 보기 , 
  ...
  //java.lang.AutoClooseable  : java.lang <====  import 문을 사용하지 않아도 된다. 
  //java.lang.Object   

   //예외를 처리하는 방법 
     //1. try catch 를 사용해서 예외 처리     : 자신이 직접 예외를 처리 하겠다. 
     //2. throws를 통해서 예외를 처리 하는 방법  : 예외를 내가 처리하지 않고 호출하는 곳에서 예외를 처리 하도록 미룬다. 
			//예외 전가. 메소드 예외 전가 
		  //throw   : 예외를 강제로 발생시키다.   
...
class A implements AutoCloseable{
	...
	public void close() throws Exception {   //close() 메소드가 자동으로 호출 된다. 
		...
		if (resource != null) {
			resource = null ; 
			System.out.println("리소스가 해재 되었습니다"); 
			System.out.println(resource);
		}		
	}	
}
